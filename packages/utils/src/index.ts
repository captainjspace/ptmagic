...
export const defaultConfig: TokenCalculatorConfig = { model: "flashLite31", inputTokens: { prompt: { tokens: 12000, isContextCached: true }, contextHistory: { tokens: 4000 } }, outputTokens: { average: 250, peak: 4400 }, agentPattern: { name: "Single Agent Double Loop", contextFactor: 2 }, timeFactors: { turnDuration: 6, turnsPerMinute: 3 }, site: { dailyUsers: 250000, canary: 0.05, rush: 0.2 } }; 
...
export const calcTokens = (inputConfig: TokenCalculatorConfig): TokenCalcResponse => {
  ...
  const decoratedGSUModel = getDecorateGSUModel( inputConfig.model);
  const tokenPack = crunchTokens(inputConfig, decoratedGSUModel);
  const userCapacity = getUserCapacity(inputConfig,decoratedGSUModel,tokenPack.AdjustedTokenInputs);
  
  const rushHourCapacity  = calculateRushHour({decoratedSiteModel, decoratedGSUModel, userCapacity, adjustedTokens: tokenPack.AdjustedTokenInputs});
  return  { inputConfig, decoratedGSUModel, decoratedSiteModel, tokenPack, userCapacity, rushHourCapacity }
...
export const tables = (calcData: unknown) => console.table(flattenObject(calcData));
export const getCalcAsJSON = (calcData: unknown) => JSON.stringify(calcData, null, 4);
