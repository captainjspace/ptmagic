...
export const getUserCapacity = (
  ...
  const outputAverageAdjustedTokens =
    adjustedTokens.outputAverageAdjustedTokens;
  ...
  const maxConcurrentInputUsers = Math.floor(
    tokenBucketSize / burndownInputTokens,
  );
  ...
  const staggeredMinuteBurn =
    burndownTokens * turnsPerMinute * maxConcurrentInputUsers;
  const userCapacityRatio =
    maxConcurrentInputUsers / (staggeredMinuteBurn / perMinute);
  ...
  const peakStableBurn =
    (stableBurn / (realUserFloor || 1)) * Math.max(realUserFloor - 1, 0) +
    p95TokenBurn;
  ...
  return {
    ...
    remainingTokens,
    AtTwenty,
    userCapacity: userCapacityRatio,
    realUserFloor,
    stableBurn,
    stableRecovery,
    turnDuration,
