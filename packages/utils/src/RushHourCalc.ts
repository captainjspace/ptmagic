...
function makeSimulator(
  ...
  return {
    ...
    setStacking() {
      this.stacking = this._debtStack.length > 0;
    },
    ...
    burnReserve(tokens: number) {
      ...
      if (this.reserveTokens <= 0)
        throw Error(getErrorCode(severityKey, subjectKey));
...
export const calculateRushHour = (data: {
  ...
  for (let i = 1; i <= rushDaily - realUserFloor; i++) {
    ...
    minuteGrid[i] = rampSim.stackCalc({
      tokens: minuteBurn,
      activeUsers,
      isPeak: false,
    });
    if (minuteGrid[i]!.reserveTokens > tokenBucketSize / 2)
      maxStableUsers = activeUsers;
  ...
  for (let min = 1; min <= 60; min++) {
    const isPeak =
      min >= peakStart && min < peakStart + peakDuration ? true : false;
    const activeUsers = Math.min(realUserFloor + min, maxStableUsers);

    const minuteBurn =
      activeUsers * burndownTokens * turnsPerMinute +
      (isPeak ? p95AdditionalBurn : 0);
    hourGrid[min] = hourSim.stackCalc({
      tokens: minuteBurn,
      activeUsers,
      isPeak,
    });
