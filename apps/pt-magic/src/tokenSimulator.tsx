  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 p-8 font-sans">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        {/* Header */}
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white">GSU Token Burst & Recovery</h1>
          <p className="text-slate-400 mt-2">Modeling 1x Gemini 2.5 Flash GSU (2,690 TPS Baseline / 322k Burst Bucket)</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl">
            <h2 className="text-xl font-semibold mb-6 text-blue-400">Environment Variables</h2>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className={`p-4 rounded-lg border flex items-center justify-between ${isSteadyFailing ? 'bg-red-950/50 border-red-900 text-red-200' : 'bg-emerald-950/30 border-emerald-900/50 text-emerald-200'}`}>
              <div>
                <h3 className="font-bold">{isSteadyFailing ? 'CRITICAL: Baseline Exceeded' : 'Baseline Stable'}</h3>
                <p className="text-sm opacity-80">
                  {isSteadyFailing ? 'Your steady state users are pulling more than 2,690 TPS. The system will crash without spillover.' : 'Steady state leaves enough faucet capacity to refill the bucket after bursts.'}
                </p>
              </div>
              <div className="text-2xl font-bold tracking-tighter">
                {Math.round(steadyTPS).toLocaleString()} <span className="text-sm font-normal opacity-70">TPS</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 relative overflow-hidden">
                ...
                <h3 className="text-slate-400 font-medium mb-1">During Burst Spike ({steadyUsers + burstUsers} users)</h3>
                <div className="text-3xl font-bold text-white mb-4">
                  {Math.round(burstTPS).toLocaleString()} <span className="text-lg text-slate-500 font-normal">TPS</span>
                </div>
                
                <div className="bg-slate-950 rounded p-3">
                  <div className="text-sm text-slate-400">Bucket Drains In</div>
                  <div className={`text-xl font-bold ${!isBurstDraining ? 'text-emerald-400' : (timeToEmptySeconds < 30 ? 'text-red-400' : 'text-amber-400')}`}>
                    {!isBurstDraining ? "Safe (No Drain)" : formatTime(timeToEmptySeconds)}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 relative overflow-hidden">
                ...
                <h3 className="text-slate-400 font-medium mb-1">Post-Burst Recovery</h3>
                <div className="text-3xl font-bold text-white mb-4">
                  {Math.round(recoveryRate > 0 ? recoveryRate : 0).toLocaleString()} <span className="text-lg text-slate-500 font-normal">TPS Excess</span>
                </div>
                
                <div className="bg-slate-950 rounded p-3">
                  <div className="text-sm text-slate-400">Bucket Refills In</div>
                  <div className={`text-xl font-bold ${isSteadyFailing ? 'text-red-500' : 'text-blue-400'}`}>
                    {isSteadyFailing ? "Never (Baseline Heavy)" : formatTime(timeToRecoverSeconds)}
                  </div>
                </div>
              </div>
