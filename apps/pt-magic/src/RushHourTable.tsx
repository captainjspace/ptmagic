    ...
    return (
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-8 text-center">
        <p className="text-slate-400 italic">
          No rush hour data available for the current configuration.
        </p>
      </div>
    );
  }

  const rows = Object.entries(minuteGrid).sort(
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
        <div className="p-6 border-b border-slate-800 bg-slate-900/50">
          ...
          <p className="text-sm text-slate-400 mt-2">
            Simulating one additional user arriving every minute beyond baseline
            capacity. Shows how long the burst bucket survives under sustained
            overage.
          </p>
        </div>
            ...
            <tbody className="divide-y divide-slate-800">
              {rows.map(([minute, record]) => {
                ...
                return (
                  <tr
                    key={minute}
                    className={`hover:bg-slate-800/30 transition-colors ${isCritical ? "bg-red-950/10" : ""}`}
                  >
                    ...
                    <td className="px-6 py-4 font-mono">
                      {record.minuteBurn.toLocaleString()}
                    </td>
                    <td className="px-6 py-4 font-mono text-amber-300">
                      {record.accumulatedTokenDebt.toLocaleString()}
                    </td>
                      ...
                      <div className="flex items-center space-x-2">
                        <div className="w-24 h-2 bg-slate-800 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${Math.max(0, (record.reserveTokens / data.decoratedGSUModel.gsuModelCalcs.tokenBucketSize) * 100)}%`,
                            }}
                          ></div>
                        </div>
                        <span className="font-mono">
                          {record.reserveTokens.toLocaleString()}
                        </span>
                      </div>
                    </td>
                    <td className="px-6 py-4 font-mono text-slate-400">
                      {record.recoveryTime}s
                    </td>
                    <td className="px-6 py-4">
                      {record.reserveTokens <= 0 ? (
                        <span className="px-2 py-1 bg-red-900/40 text-red-400 rounded-md text-xs font-bold uppercase border border-red-800">
                          Exhausted
                        </span>
                      ) : record.stacking ? (
                        <span className="px-2 py-1 bg-amber-900/40 text-amber-400 rounded-md text-xs font-bold uppercase border border-amber-800">
                          Stacking Debt
                        </span>
                      ) : (
                        <span className="px-2 py-1 bg-emerald-900/40 text-emerald-400 rounded-md text-xs font-bold uppercase border border-emerald-800">
                          Stable
                        </span>
                      )}
                    </td>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
        ...
        <div className="bg-slate-900/50 border border-slate-800 p-4 rounded-xl shadow-lg border-l-4 border-l-purple-500">
          ...
          <span className="text-xl font-black text-purple-400">
            {rows[rows.length - 1]?.[1].activeUsers} Users
          </span>
        </div>
        <div className="bg-slate-900/50 border border-slate-800 p-4 rounded-xl shadow-lg border-l-4 border-l-amber-500">
          ...
          <span className="text-xl font-black text-amber-400">
            {Math.round(
              data.userCapacity.stableBurn /
                data.userCapacity.realUserFloor /
                1000,
            ).toLocaleString()}
            k
          </span>
      ...
      {hourGrid && Object.keys(hourGrid).length > 0 && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
          <div className="p-6 border-b border-slate-800 bg-slate-900/50">
            ...
            <p className="text-sm text-slate-400 mt-2">
              60-minute hour at baseline load. One p95 user arrives at minute{" "}
              {peakStart} and burns for 3 turns. Statistically ~1 occurrence per
              hour.
            </p>
          </div>
          <div className="overflow-x-auto">
                  ...
                  .map(([minute, record]) => (
                    ...
                    >
                      <td className="px-6 py-4 font-mono text-purple-400">
                        {minute}m
                        {record.isPeak && (
                          <span className="ml-2 text-[10px] bg-purple-800 text-purple-200 px-1 rounded">
                            PEAK
                          </span>
                        )}
                      </td>
                      ...
                      <td className="px-6 py-4">
                        {record.isPeak ? (
                          <span className="px-2 py-1 bg-purple-900/40 text-purple-400 rounded-md text-xs font-bold uppercase border border-purple-800">
                            p95 Burst
                          </span>
                        ) : record.stacking ? (
                          <span className="px-2 py-1 bg-amber-900/40 text-amber-400 rounded-md text-xs font-bold uppercase border border-amber-800">
